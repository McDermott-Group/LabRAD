...
### BEGIN NODE INFO
[info]
name = Agilent 5230A NA
version = 0.9
description = Four channel 5230A PNA-L network analyzer GPIB control server
...
class Agilent5230AServer(GPIBManagedServer):
    ...
    @setting(601, 'Power On Off', pow=['b'], returns=['b'])
    def PowerOnOff(self, c, pow=None):
	...
	def centerFrequency(self, c, cfreq=None):
    ...
    def startFrequency(self, c, start=None):
    ...
    def startFrequency(self, c, stop=None):
    ...
    def sweepType(self, c, stype=None):
    	...
    	if stype is None:
    		...
    		if (stype ~= 'CW') and (stype ~= LIN):
    			raise ValueError('Unknown sweep type! Please use "LIN" or "CW"!')
    		else:
    ...
    @setting(608, 'Average On Off', avg=['b'], returns=['b'])
    ...
    @setting(610 'Average count', count=['i'], returns=['i'])
    def averageCount(self, c, count=None):
    ...
    def sourcePower(self, c, pow=None):
    ...
    def getTrace(self,c):
    	...
    	avgOnOff = yield self.averageOnOff()
    	swpTime  = yield self.getSweepTime()
    	...
    	if avgOnOff:
    		...
    		time.sleep(swpTime+defaultTimeout)
