...
class LBRFGenServer(LabradServer):
    ...
    def refreshRFGenerators(self):
        ...
        if n == self._num_devs:
            ...
            for idx in range(self._num_devs):
                ...
                power = yield self.set_power(self._pseudo_context)
    ...
    @setting(565, 'Select Attenuator', SN='w', returns='')
    ...
    @setting(566, 'Deselect Attenuator', returns='')
    ...
    @setting(545, 'Set Power', power=['v[dB]'], returns=['v[dB]'])		
    def set_power(self, c, power=None):
    ...
    def max_power(self, c):
        ...
        max_attn = 0.25 * (yield self.VNXdll.fnLMS_GetMaxPwr(DID))
        yield self.VNXdll.fnLDA_CloseDevice(DID)
        returnValue(units.Value(max_attn, 'dBm'))
    ...
    def min_power(self, c):
        ...
        DID = ctypes.c_uint(self.getDeviceDID(c))
        yield self.VNXdll.fnLDA_InitDevice(DID)
        min_attn = 0.25 * (yield self.VNXdll.fnLMS_GetMinPwr(DID))
        yield self.VNXdll.fnLDA_CloseDevice(DID)
        returnValue(units.Value(min_attn, 'dBm'))
