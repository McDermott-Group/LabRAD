import Tkinter
from Tkinter import Tk, Frame, BOTH, OptionMenu, Canvas, Button, Checkbutton, Label, StringVar, IntVar, Entry
import labrad
...
with labrad.connect() as cxn:
    ...
    class DCRackGui(Frame):
            ...
            for ii in range(0, len(self.availableCards)):
                ...
                for jj in range(0, len(self.PreampChannels)):
                    ...
                    for kk in range(0, len(self.PreampSettings)):
                        self.PreampSettingsData[self.availableCards[ii]][self.PreampChannels[jj]][self.PreampSettings[kk]]=""
        ...
        def busSettingControls(self):
            ...
            for jj in range(0, len(self.BusChannels)):
                ...
                for kk in range(0, len(self.BusSettings)):
                    ...
                    #self.BusOptionMenus[self.BusChannels[jj]][self.BusSettings[kk]].configure(state=configState)
        
        def setBusSetting(self, busName, settingName, settingVal):
            print "setBusSetting called with settingName=",settingName
        ...
        def preampSettingControls(self, cardSelection):
            print "preampSettingControls called"
            ...
            for jj in range(0, len(self.PreampChannels)):
                ...
                for kk in range(0, len(self.PreampSettings)):
                    ...
                    if self.PreampSettings[kk] not in self.PreampOptionMenusData[self.PreampChannels[jj]].keys():
                        self.PreampOptionMenusData[self.PreampChannels[jj]][self.PreampSettings[kk]]=StringVar() #if "Offset" not in self.PreampSettings[kk]: else IntVar()
                        if "Offset" not in self.PreampSettings[kk]:
                           ...
                           self.PreampOptionMenus[self.PreampChannels[jj]][self.PreampSettings[kk]]=Entry(self, textvariable=self.PreampOptionMenusData[self.PreampChannels[jj]][self.PreampSettings[kk]])
                           ...
                           self.PreampOptionMenus[self.PreampChannels[jj]][self.PreampSettings[kk]].bind("<FocusOut>",lambda event, channel= self.PreampChannels[jj],settingName=self.PreampSettings[kk],settingVal=self.PreampOptionMenusData[self.PreampChannels[jj]][self.PreampSettings[kk]]:self.setPreampOffsetFocusOut(channel, settingName, settingVal))
        ...
        def setPreampSetting(self, channel, settingName, settingVal):
            print "setPreampSetting called with:"
            print "  channel=", channel
            print "  settingName=", settingName
            print "  settingVal=", settingVal.get()
        ...
        def changeHighPassFilterSetting(self, cardID, channel, highPassSetting):
            print "changeHighFilterSetting"
            dc.select_card(cardID)
            dc.change_high_pass_filter('A', highPassSetting)

        def changeLowPassFilterSetting(self, cardID, channel, highPassSetting):
            print "changeHighFilterSetting"
            dc.select_card(cardID)
            dc.change_low_pass_filter('A', highPassSetting)

        def setPreampOffsetReturn(self, channel, settingName, settingVal):
            print "setPreampOffsetReturn"
            self.focus()
        
        def setPreampOffsetFocusOut(self, channel, settingName, settingVal):
            print "setPreampOffsetFocusOut"
            self.setPreampSetting(channel, settingName, settingVal)

        def initDacVrefs(self):
            print "initializing DAC Vrefs"
            dc.init_dacs()
        
        def saveToRegistry(self):
            print "saving settings to registry"
        
        def uploadFromRegistry(self):
            print "upload settings from registry"

        def setBusState(self, busName, busState):
            print "setBusState called with:"
            print "  busName=", busName
            print "  busState=", busState.get()
            state = busState.get()
        ...
        def setBusCardMapping(self, busName, cardSelection):
            print "setBusCardMapping called with:"
            print "busName=", busName
            print "cardSelection=", cardSelection.get()
        ...
        def setBusChannelMapping(self, busName, cardSelection, channelSelection):
            print "setBusChannelMapping called with:"
            print "  busName=", busName
            print "  cardSelection=", cardSelection.get()
            selection = channelSelection.get()
            card = cardSelection.get()
            print "  channelSelection=", channelSelection.get()
            state = self.BusOptionMenusData[busName]["On/Off"].get()
            print "  busState=", state
            cardId = int(card.split(",")[0].strip("("))
            print "  cardId=", cardId
