...
class DoubleJPMCorrelationSIM928(JPMExperiment):
    ...
    def run_once(self, histogram=False, plot_waveforms=False):
        ...
        # if 'JPM A Fast Pulse' in requested_waveforms:
            # waveforms['JPM A Fast Pulse'] = np.hstack([pulse.DC(DAC_ZERO_PAD_LEN + RFtoFP_A, 0),
                                                      # JPMA_FP,
                                                      # pulse.DC(RF_time - RFtoFP_A - JPMA_FP.size, -1),
                                                      # pulse.DC(DAC_ZERO_PAD_LEN, 0)])

        # if 'JPM B Fast Pulse' in requested_waveforms:
            # waveforms['JPM B Fast Pulse'] = np.hstack([pulse.DC(DAC_ZERO_PAD_LEN + RFtoFP_A + JPMdelay, 0),
                                                      # JPMB_FP,
                                                      # pulse.DC(RF_time - RFtoFP_A - JPMdelay - JPMB_FP.size, -1),
                                                      # pulse.DC(DAC_ZERO_PAD_LEN, 0)])
        ...
        P = [[], []]
        JPMA_BV = self.value('JPM A Bias Voltage')
        JPMB_BV = self.value('JPM B Bias Voltage')
        for k in range(self.value('Reps')):
            self.send_request('JPM A Bias Voltage')
            self.send_request('JPM B Bias Voltage')
            self.acknowledge_requests()
            result = fpga.load_and_run(dac_srams, mems, 30)
            self.value('JPM A Bias Voltage', 0 * units.V, False)
            self.value('JPM B Bias Voltage', 0 * units.V, False)
            self.send_request('JPM A Bias Voltage')
            self.send_request('JPM B Bias Voltage')
            self.value('JPM A Bias Voltage', JPMA_BV, False)
            self.value('JPM B Bias Voltage', JPMB_BV, False)
            P[0].append(result[0][0])
            P[1].append(result[1][0])
            self.acknowledge_requests()
