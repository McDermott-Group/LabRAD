...
class DoubleJPMCorrelationSIM928(JPMExperiment):
    ...
    def run_once(self, histogram=False, plot_waveforms=False):
        ...
        for k in range(self.value('Reps')):
            self.send_request('JPM A Bias Voltage', JPMA_BV)
            self.send_request('JPM B Bias Voltage', JPMB_BV)
            self.acknowledge_requests()
            result = self.boards.run(30)
            self.send_request('JPM A Bias Voltage', 0 * units.V)
            self.send_request('JPM B Bias Voltage', 0 * units.V)
