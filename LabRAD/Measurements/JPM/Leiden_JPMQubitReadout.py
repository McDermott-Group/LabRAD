...
ExptVars = {
            'Reps': 10000, # should not exceed ~55,000
            ...
            'Qubit Frequency': 4.1865 * GHz,
            'Qubit Power': 13 * dBm,
            'Qubit Attenuation': 5 * dB, # should be in (0, 63] range
            ...
            'Qubit Flux Bias Voltage': 0.8 * V,

            'Readout Frequency': 4.9103 * GHz,
            'Readout Power': 10 * dBm,
            'Readout Attenuation': 19 * dB, # should be in (0, 63] range
...
# with jpm_qubit_experiments.JPMStarkShift() as run:
    ...
    run.sweep('Qubit Frequency', np.linspace(4.1, 4.9, 801) * GHz,
            save=True, print_data='Switching Probability', plot_data='Switching Probability')
            
    # run.sweep(['Qubit Attenuation', 'Qubit Frequency'], 
            # [np.linspace(1, 21, 11) * dB, np.linspace(4.45, 4.65, 101) * GHz],
    ...
    # run.sweep('Qubit Drive to Readout', np.linspace(0, 1000, 251) * ns,
            # save=True, print_data='Switching Probability', plot_data='Switching Probability')
    ...
    # run.sweep(['Readout Attenuation', 'Readout Frequency'], 
            # [np.linspace(10, 30, 11) * dB, np.linspace(4.905, 4.925, 51) * GHz],
            # save=True, print_data=['Switching Probability'])
