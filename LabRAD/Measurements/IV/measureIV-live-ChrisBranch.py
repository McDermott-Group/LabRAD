...
class MeasureIV(tk.Tk):   
    ...
    def initWindow(self):
        ...
        self.enterz = {}
        self.labelCtrl = {}
        self.labelCtrl['measurementType'] = [tk.Label(self,text="Measurement Type:"),1,0]
        self.labelCtrl['deviceType'] = [tk.Label(self,text="Device Type:"),2,0]
        self.labelCtrl['waferName'] = [tk.Label(self,text="Wafer Name:"),2,1]
        self.labelCtrl['dieName'] = [tk.Label(self,text="Die Name:"),2,2]
        self.labelCtrl['measurementLocation'] = [tk.Label(self,text="Measurement Loc:"),2,3]
        self.labelCtrl['outputWaveFreq'] = [tk.Label(self,text="Output Wave Frequency (Hz):"),5,2]
        
        for key in self.labelCtrl:
            self.enterz[key] = tk.Entry(self,width="10")
        ...
        self.enterz['comments'] = tk.Entry(self,width="110")
        ...
        tk.Label(self,text="Comments:").grid(row=3,column=0)
        ...
        #self.buttonz['stopExperiment'] = tk.Button(master=self,text="Close Experiment",command=self.stopCollection)
        self.buttonz['stopAverage'] = tk.Button(master=self,text="Stop Average",command=self.stopAverage)
    ...
    def collect_data(self):
        ...
        while (self.running):
            ...
            #list = newdata
            #list.sort()
            #list = self.removeOutliers(newdata)
            list = newdata
    ...
    def orderPoints(self, points, base_min):
        ...
        for i in l:
            #having a better guess for base_min would fix this
            dist = base_min
            p = pts[index]
            next_ind = -1
    ...
    def calcAverage(self):
        ...
        for i in range(min):
            ...
            for j in range(g_len):              
                ...
                points.append([self.graphs[j][i][0],self.graphs[j][i][1]])
        ...
        l = range(int(len(points) / 10))
        means = []
        for i in l:
            ind = int(i * 10 * np.random.rand())
            print ind
            means.append(points[ind])
        
        #this 20 quantity may have to be adjusted mannually
        base_min = max((Xmax - Xmin) / 20.0,(Ymax - Ymin) / 20.0)**2
        print base_min
                 
        p_min = 0
        dist = 0 
        '''
        #clustering this many points takes way too long
        #tries to converge after 10 iterations
        ...
        print len(means) #fix algo or if < g_len * 900 redo
...
#        self.bncMarkedUpdate = True
    ...
    def setupBNC(self):
        ...
        for i in self.topLevel['selects']:
            ...
            self.topLevel['selects'][i]['sv'].set(self.topLevel['selects'][i]['opt'][0])
    ...
    def saveConf(self, additional):
        ...
        for key in additional:
            self.oldConf[key] = expInfo[key]
            expInfo[key] = additional[key]
            if expInfo[key] is not self.oldConf[key]:
                self.bncMarkedUpdate = True
        try:
            #should maybe put this outside this function
            self._tl.destroy()
        except Exception as e:
            print '\n'
