...
class MeasureIV(tk.Tk):   
    ...
    def initializeWaves(self):
        sampRate = self.sampRate.get()
        buffsize = int(float(sampRate) / self.ACFreq.get())
        self.data = np.zeros((buffsize,), dtype=np.float64)
        
        AItaskHandle = daqmx.TaskHandle()
        AOACtaskHandle = daqmx.TaskHandle()
        AODCtaskHandle = daqmx.TaskHandle()
        
        daqmx.CreateTask("", AItaskHandle)
        daqmx.CreateAIVoltageChan(AItaskHandle, "Dev1/ai"+str(self.portOut.get()), "", daqmx.Val_Diff, -10.0, 10.0, daqmx.Val_Volts, None)
        daqmx.CfgSampClkTiming(AItaskHandle, "", sampRate, daqmx.Val_Rising, daqmx.Val_ContSamps, buffsize)
        trigName = daqmx.GetTerminalNameWithDevPrefix(AItaskHandle, "ai/StartTrigger")

        daqmx.CreateTask("", AOACtaskHandle)
        daqmx.CreateAOVoltageChan(AOACtaskHandle, "Dev1/ao"+str(self.portACIn.get()), "",  -10.0, 10.0, daqmx.Val_Volts, None)
        daqmx.CfgSampClkTiming(AOACtaskHandle, "", sampRate, daqmx.Val_Rising, daqmx.Val_ContSamps, buffsize)
        daqmx.CfgDigEdgeStartTrig(AOACtaskHandle, trigName, daqmx.Val_Rising)

        daqmx.CreateTask("", AODCtaskHandle)
        daqmx.CreateAOVoltageChan(AODCtaskHandle, "Dev1/ao"+str(self.portDCIn.get()), "",  -10.0, 10.0, daqmx.Val_Volts, None)
        daqmx.CfgSampClkTiming(AODCtaskHandle, "", sampRate, daqmx.Val_Rising, daqmx.Val_ContSamps, buffsize)
        daqmx.CfgDigEdgeStartTrig(AODCtaskHandle, trigName, daqmx.Val_Rising)
        ...
        daqmx.WriteAnalogF64(AOACtaskHandle, self.sampRate.get(), False, 10.0, daqmx.Val_GroupByChannel, self.genWave(self.ACAmp.get(), self.ACFreq.get()))
        #daqmx.WriteAnalogF64(AODCtaskHandle, buffsize, False, 10.0, daqmx.Val_GroupByChannel, self.genWave(self.DCAmp.get(), 0))
        
        daqmx.StartTask(AOACtaskHandle)
        #daqmx.StartTask(AODCtaskHandle)
        daqmx.StartTask(AItaskHandle)
        
        self.AOACtaskHandle = AOACtaskHandle # so they can be ended when quit is called
        self.AODCtaskHandle = AODCtaskHandle
        self.AItaskHandle = AItaskHandle
    ...
    def _quit(self):
        ...
        # &&& set voltages back to 0 or they will stay wherever they were
        self.running = False
        ...
        daqmx.StopTask(self.AOACtaskHandle)
        daqmx.ClearTask(self.AOACtaskHandle)
        daqmx.StopTask(self.AODCtaskHandle)
        daqmx.ClearTask(self.AODCtaskHandle)
        #os._exit(1)
