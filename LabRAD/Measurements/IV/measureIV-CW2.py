...
class MeasureIV(tk.Tk):   
    ...
    def initParams(self):
        ...
        self.portOut.trace('w',self.changeWaveOutput)
    ...
    def initializeWaveOutput(self):
        try:
            self.wave = bnc.NIWaves(self.portACIn.get(),[self.portOut.get()],self.sampRate.get())
            self.wave.setWave(self.genWave( self.ACAmp.get(), self.ACFreq.get() ))
            self.wave.setCallback(self.updateData)
            self.wave.startWave()
    ...
    def measureIV(self):
        ...
        data_read = threading.Thread(target=self.initializeWaveOutput, args=())
        data_read.daemon = True
        data_read.start()
